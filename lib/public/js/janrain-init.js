...
(function () {
    if (_typeof(window.janrain) !== 'object') window.janrain = {};
    ...
    if (_typeof(window.janrain.settings.share) === 'object') {
        var e = document.createElement('script');
        e.type = 'text/javascript';
        e.id = 'janrainWidgets';
        if (document.location.protocol === 'https:') {
...
function janrainExampleImplementationFunctions() {
    ...
    function hideResendLink(result) {
        ...
        if (result.controlName == "resendVerificationEmail" && result.screen == "editProfile") {
